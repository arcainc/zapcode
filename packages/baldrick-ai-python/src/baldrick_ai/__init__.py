...
Works with any AI SDK:
    ...
    # Anthropic SDK
    from baldrick_ai import baldrick
    b = baldrick(tools={...})
    ...
    # Custom adapter
    from baldrick_ai import baldrick, Adapter
    class MyAdapter(Adapter):
        name = "my-sdk"
        def adapt(self, ctx):
            return {"system": ctx.system, "tool": ctx.tool_schema}
    b = baldrick(tools={...}, adapters=[MyAdapter()])
...
@dataclass
class BaldrickAI:
    """Result of `baldrick()` — adapters for every major AI SDK."""
...
def execute(
    ...
    *,
    ...
    Lower-level API if you don't need AI SDK integration::
        ...
        from baldrick_ai import execute, ToolDefinition, ParamDef
