...
//!     ▼
//! ┌─────────┐
//! │  parser  │  oxc_parser → BaldrickIR (parser/ir.rs)
//! └────┬────┘
//!      ▼
//! ┌──────────┐
//! │ compiler │  BaldrickIR → stack-based bytecode (compiler/instruction.rs)
//! └────┬─────┘
//!      ▼
//! ┌─────────┐
...
//! ## Key modules
//!
//! - [`parser`] — Walks the oxc AST and emits [`parser::ir::BaldrickIR`]
//! - [`compiler`] — Lowers IR to [`compiler::instruction::Instruction`] bytecode
//! - [`vm`] — Stack-based VM that executes bytecode; entry point is [`BaldrickRun`]
