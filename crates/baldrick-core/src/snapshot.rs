...
impl BaldrickSnapshot {
    ...
    pub(crate) fn capture(vm: &Vm) -> Result<Self> {
        ...
        let data = postcard::to_allocvec(&snapshot)
            .map_err(|e| BaldrickError::SnapshotError(format!("capture failed: {}", e)))?;
    ...
    pub fn dump(&self) -> Result<Vec<u8>> {
        postcard::to_allocvec(self)
            .map_err(|e| BaldrickError::SnapshotError(format!("dump failed: {}", e)))
    }

    /// Deserialize a snapshot from bytes.
    pub fn load(bytes: &[u8]) -> Result<Self> {
        postcard::from_bytes(bytes)
            .map_err(|e| BaldrickError::SnapshotError(format!("load failed: {}", e)))
    }
    ...
    pub fn resume(self, return_value: Value) -> Result<VmState> {
        let vm_snap: VmSnapshot = postcard::from_bytes(&self.data)
            .map_err(|e| BaldrickError::SnapshotError(format!("resume decode failed: {}", e)))?;
