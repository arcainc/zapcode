use baldrick_core::{BaldrickRun, BaldrickSnapshot, ResourceLimits, Value};
use baldrick_core::vm::VmState;

/// Helper: create a BaldrickRun with external functions and run start().
...
fn test_snapshot_dump_load_roundtrip() {
    ...
    // Load from bytes
    let loaded = BaldrickSnapshot::load(&bytes).unwrap();
