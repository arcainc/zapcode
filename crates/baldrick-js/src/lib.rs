...
use baldrick_core::{BaldrickRun, BaldrickSnapshot, ResourceLimits, Value, VmState};
...
pub struct BaldrickSuspension {
    ...
    /// Opaque snapshot bytes -- pass to `BaldrickSnapshotHandle.load()` to resume.
    pub snapshot: Buffer,
}
...
#[napi]
pub struct BaldrickSnapshotHandle {
    inner: BaldrickSnapshot,
}

#[napi]
impl BaldrickSnapshotHandle {
    ...
    pub fn load(bytes: Buffer) -> napi::Result<Self> {
        let snapshot = BaldrickSnapshot::load(&bytes)
            .map_err(|e| napi::Error::from_reason(e.to_string()))?;
    ...
    ///
    /// Returns either a `BaldrickResult` (complete) or a `BaldrickSuspension`
    /// (suspended again on another external call).
    #[napi(ts_return_type = "BaldrickResult | BaldrickSuspension")]
    pub fn resume(
        &self,
        return_value: serde_json::Value,
    ) -> napi::Result<Either<BaldrickResult, BaldrickSuspension>> {
...
impl Baldrick {
    #[napi(constructor)]
    pub fn new(code: String, options: Option<BaldrickOptions>) -> napi::Result<Self> {
        let opts = options.unwrap_or(BaldrickOptions {
            inputs: None,
    ...
    pub fn run(
        ...
        match result.state {
            VmState::Complete(v) => Ok(BaldrickResult {
                completed: true,
...
/// Convert a JS inputs map to the `Vec<(String, Value)>` that baldrick-core expects.
...
/// Convert a `serde_json::Value` to a `baldrick_core::Value`.
...
/// Convert a `baldrick_core::Value` to a `serde_json::Value`.
...
/// Package a `VmState` into either a `BaldrickResult` or `BaldrickSuspension`.
fn vm_state_to_either(
    state: VmState,
    stdout: String,
) -> napi::Result<Either<BaldrickResult, BaldrickSuspension>> {
    match state {
        VmState::Complete(v) => Ok(Either::A(BaldrickResult {
            completed: true,
            output: value_to_json(&v),
            stdout,
        })),
        VmState::Suspended {
            ...
            let snap_bytes = snapshot
                .dump()
                .map_err(|e| napi::Error::from_reason(e.to_string()))?;
            Ok(Either::B(BaldrickSuspension {
                completed: false,
                function_name,
