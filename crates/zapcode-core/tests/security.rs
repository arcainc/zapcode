...
fn test_stack_overflow_direct() {
    ...
    assert!(
        matches!(err, ZapcodeError::StackOverflow(_) | ZapcodeError::TimeLimitExceeded),
...
fn test_infinite_while_loop() {
    ...
    let err = result.unwrap_err();
    assert!(
        matches!(err, ZapcodeError::TimeLimitExceeded | ZapcodeError::AllocationLimitExceeded),
...
fn test_stack_trace_no_host_info() {
    ...
    assert!(result.is_err());
    let err_msg = format!("{}", result.unwrap_err());
    assert!(!err_msg.contains("zapcode-core"), "VULN: error leaks crate name: {err_msg}");
