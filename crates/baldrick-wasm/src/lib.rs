...
// ---------------------------------------------------------------------------
// Value conversion: baldrick_core::Value <-> JsValue
// ---------------------------------------------------------------------------

/// Convert a `JsValue` to a `baldrick_core::Value`.
fn js_to_value(js: &JsValue) -> Result<Value, JsError> {
    if js.is_undefined() {
        ...
        Err(JsError::new(&format!(
            "cannot convert JS value to Baldrick value: {:?}",
            js
        )))
    }
}

/// Convert a `baldrick_core::Value` to a `JsValue`.
...
/// Convert a `BaldrickError` to a `JsError`.
fn baldrick_err(e: BaldrickError) -> JsError {
...
// ---------------------------------------------------------------------------
// Baldrick — main entry point
// ---------------------------------------------------------------------------

#[wasm_bindgen]
pub struct Baldrick {
    inner: baldrick_core::BaldrickRun,
}

#[wasm_bindgen]
impl Baldrick {
    /// Create a new Baldrick instance.
    ///
    ...
    pub fn new(code: &str, options: JsValue) -> Result<Baldrick, JsError> {
        let opts: BaldrickOptions = if options.is_undefined() || options.is_null() {
            BaldrickOptions::default()
        } else {
        ...
        let inner = baldrick_core::BaldrickRun::new(
            code.to_string(),
            opts.inputs,
            opts.external_functions,
            limits,
        )
        .map_err(baldrick_err)?;
    ...
    pub fn run(&self, inputs: JsValue) -> Result<JsValue, JsError> {
        ...
        let result = self.inner.run(input_values).map_err(baldrick_err)?;
    ...
    pub fn start(&self, inputs: JsValue) -> Result<JsValue, JsError> {
        let input_values = extract_inputs(&inputs)?;
        let state = self.inner.start(input_values).map_err(baldrick_err)?;
...
fn vm_state_to_js(state: VmState, stdout: &str) -> Result<JsValue, JsError> {
    ...
    match state {
        ...
        VmState::Suspended {
            ...
            let snap = BaldrickSnapshot { inner: snapshot };
            Reflect::set(
                &obj,
...
// ---------------------------------------------------------------------------
// BaldrickSnapshot — snapshot / resume
// ---------------------------------------------------------------------------
...
impl BaldrickSnapshot {
    ...
    pub fn dump(&self) -> Result<Vec<u8>, JsError> {
        self.inner.dump().map_err(baldrick_err)
    }

    /// Deserialize a snapshot from bytes.
    pub fn load(bytes: &[u8]) -> Result<BaldrickSnapshot, JsError> {
        let inner = CoreSnapshot::load(bytes).map_err(baldrick_err)?;
        Ok(Self { inner })
    }
