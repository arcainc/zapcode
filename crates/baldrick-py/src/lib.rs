...
// ---------------------------------------------------------------------------
// Value conversion: baldrick_core::Value <-> Python object
// ---------------------------------------------------------------------------

/// Convert a Python object to a `baldrick_core::Value`.
fn py_to_value(obj: &Bound<'_, PyAny>) -> PyResult<Value> {
    if obj.is_none() {
        ...
        Err(PyRuntimeError::new_err(format!(
            "cannot convert Python type '{}' to Baldrick value",
            obj.get_type().name()?
        )))
    }
}

/// Convert a `baldrick_core::Value` to a Python object.
...
/// Convert a `BaldrickError` to a Python `RuntimeError`.
fn baldrick_err(e: BaldrickError) -> PyErr {
...
fn run_result_to_py(py: Python<'_>, state: VmState, stdout: &str) -> PyResult<PyObject> {
    ...
    match state {
        ...
        VmState::Suspended {
            ...
            dict.set_item("snapshot", BaldrickSnapshot { inner: snapshot })?;
            dict.set_item("stdout", stdout)?;
        }
    }
    Ok(dict.into_pyobject(py)?.into_any().unbind())
...
impl BaldrickSnapshot {
    ...
    ///
    /// Returns:
    ///     A dict with either "output" or "suspended" keys (same shape as Baldrick.run()).
...
#[pymodule]
fn baldrick(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Baldrick>()?;
    m.add_class::<BaldrickSnapshot>()?;
    Ok(())
}
